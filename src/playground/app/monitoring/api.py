...
def set_exercise_objective_complete(exercise_id: uuid.UUID, objective_id: uuid.UUID) -> None:
    """
    ...
    # Check if the request was successful
    if response.status_code != 200:
        raise ValueError(f"Failed to mark objective as complete: {response.text}")
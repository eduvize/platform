...
async def connect(sid: str, environment_data: dict, extra: Optional[dict] = None):
    ...
    try:
        ...
        await socket.enter_room(sid, session_id)
        ...
        if user_id is not None:
            ...
            if is_alive is not None:
                await socket.emit("instance_connected", room=session_id) # Notify the user of the instance connection
                await socket.emit("user_connected", room=session_id) # Notify the instance of the user connection
...
async def disconnect(sid: str):
    async with socket.session(sid) as session:
        ...
        if user_id is not None:
            ...
            await socket.emit("user_disconnected", room=session_id) # Notify the instance
        
        if instance_hostname is not None and session_id is not None:
            logger.info(f"Instance {instance_hostname} disconnected")
            await socket.emit("instance_disconnected", room=session_id) # Notify the user
...
async def terminal_input(sid: str, t_input: str):
    async with socket.session(sid) as session:
        ...
        await socket.emit("terminal_input", t_input, room=session_id)
...
async def terminal_output(sid: str, output: str):
    async with socket.session(sid) as session:
        ...
        await socket.emit("terminal_output", output, room=session_id)
...
async def terminal_resize(sid: str, data: dict):
    async with socket.session(sid) as session:
        ...
        await socket.emit("terminal_resize", data, room=session_id)
